...
async function addToDB(paths: Set<string>) {
  try {
      ...
      try {
        ...
        if (!album) {
          ...
          if (picture) {
            ...
            imagePath = path.join(
              imagePath,
              `${metadata.common.album}.${extension}`
            );
        ...
        const songObj = constructSongObj(
          sPath,
          metadata,
          album.title,
          album._id,
    ...
    if (batchSongs.length > 0) Song.collection.insertMany(batchSongs);
  } catch (error) {
...
async function scanAndUpdateDB() {
  try {
    ...
    await Walk.walk(
      path.join("src", "songs"),
      (err: any, pathname: any, dirent: any) =>
    ...
    const pathsToAdd: Set<string> = new Set(
      [...localPaths].filter((x) => !dbPaths.has(x))
    );
    const pathsToRemove: Set<string> = new Set(
      [...dbPaths].filter((x) => !localPaths.has(x))
    );
